...
# Initialisation du modèle
    ...
    x = layers.Dropout(0.3)(x)
    pred = layers.Dense(7, activation="softmax")(x)

        # We use the keras Functional API to create our keras model

    model_VGG16_7 = Model(inputs=inputs , outputs=pred)
    ...
    model_VGG16_7.compile(loss='categorical_crossentropy',
                    optimizer=adam,
                    metrics=['Precision'])

    MODEL = "model_VGG16_6.keras"
    ...
    history_3 = model_VGG16_7.fit(
            X,y,
            epochs=15,
            validation_split=0.3,
            callbacks = [modelCheckpoint, LRreducer, EarlyStopper])

    model_VGG16_7.save(f"models/model_tests/model_VGG16_7class_NM_{b}_{a}.keras")

    save_model(model_VGG16_7,history_3)
